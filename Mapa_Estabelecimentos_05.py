...
caminho_hectares = os.path.join(
    base_dir,
    "Hectares.xlsx"
)

df_hec = pd.read_excel(
    caminho_hectares,
    header=None
)

df_hec = df_hec.iloc[7:].copy()
...
df_hec["Hec_Nao_Familiar"] = pd.to_numeric(
    df_hec["Hec_Nao_Familiar"],
    errors="coerce"
).fillna(0)

df_hec["Hec_Familiar"] = pd.to_numeric(
    df_hec["Hec_Familiar"],
    errors="coerce"
).fillna(0)
...
camadas = [
    ...
    ("População", "Populacao"),
    ("Área Total (ha)", "Hec_Total"),
    ("Área Familiar (ha)", "Hec_Familiar"),
    ("Área Não Familiar (ha)", "Hec_Nao_Familiar")
]
...
<b>Área Familiar:</b>
{f"{int(row['Hec_Familiar']):,.0f}".replace(",", ".") if pd.notna(row['Hec_Familiar']) else "0"} ha

<br>

<b>Área Não Familiar:</b>
{f"{int(row['Hec_Nao_Familiar']):,.0f}".replace(",", ".") if pd.notna(row['Hec_Nao_Familiar']) else "0"} ha

<br>

<b>Área Total:</b>
{f"{int(row['Hec_Total']):,.0f}".replace(",", ".") if pd.notna(row['Hec_Total']) else "0"} ha

<br>

<b>% Área Familiar:</b>
{row['Perc_Hec_Familiar']:.1f}%
